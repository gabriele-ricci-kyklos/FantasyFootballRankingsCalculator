...
{
    public class RankingsBLL : BaseWebBLL, IRankingsBLL
    {
        ...
        private PlayerStats CalculatePlayerStats(string name, HtmlNode nameNode, HtmlNode rankingNode, IList<HtmlNode> dataNodes, IList<HtmlNode> penaltyNodes)
        {
            ...
            CardType? card = GetCardForPlayer(nameNode);
        ...
        private CardType? GetCardForPlayer(HtmlNode nameNode)
        {
            nameNode.AssertNotNull("nameNode");

            bool hasYellowCard = HasCard(nameNode, CardType.YellowCard);
            if(hasYellowCard)
            {
                return CardType.YellowCard;
            }

            bool hasRedCard = HasCard(nameNode, CardType.RedCard);
            if (hasRedCard)
            {
        ...
        private bool HasCard(HtmlNode nameNode, CardType cardType)
        {
            nameNode.AssertNotNull("nameNode");
            return 
                nameNode
                    .ChildNodes
                    .SelectByClass("span", (cardType == CardType.YellowCard) ? "cart-giallo" : "cart-rosso")
                    .Any();
        }
